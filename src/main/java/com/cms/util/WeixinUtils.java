...
public class WeixinUtils {
     ...
     * @return
     */
    public static Map<String, String> wechatPay(String orderSn,BigDecimal amount, String subject, String openId, HttpServletRequest httpRequest, HttpServletResponse httpResponse) {
        Config config = SystemUtils.getConfig();
        Map<String, String> params = wechatUnifiedorder(orderSn,config.getWeixinpayAppId(), config.getWeixinpayMchId(), openId, subject, amount.multiply(new BigDecimal(100))
                .intValue(), IPUtils.getIpAddress(httpRequest));
     ...
     * @return
     */
    public static Map<String, String> h5Pay(Long ordetId, String orderNo, BigDecimal amount, String subject, HttpServletRequest httpRequest) {
        Config config = SystemUtils.getConfig();
        Map<String, String> params = h5Unifiedorder(orderNo, config.getWeixinpayAppId(), config.getWeixinpayMchId(), subject, amount.multiply(new BigDecimal(100)).intValue(),
                IPUtils.getIpAddress(httpRequest));
        ...
        String redirectUrl = new StringBuffer("").append(config.getSiteUrl()).append("/member/order/detail/").append(ordetId).toString();
        // 确认支付过后跳的地址,需要经过urlencode处理 ${base}/member/order/detail/
        try {
            mweb_url = mweb_url + "&redirect_url=" + URLEncoder.encode(redirectUrl, "UTF-8");
     ...
     * @return
     */
    public static Map<String, String> wechatUnifiedorder(String orderSn,String appid, String mch_id, String openid, String body, int total_fee, String spbill_create_ip) {
        Config config = SystemUtils.getConfig();
        Map<String, String> params = new HashMap<String, String>();
        ...
        params.put("attach", orderSn);
        params.put("out_trade_no", generateUUID());
        params.put("total_fee", total_fee + "");
        params.put("spbill_create_ip", spbill_create_ip);
        params.put("notify_url", config.getSiteUrl() + "/payment/weixinNotify");
     ...
     * h5支付统一下单
     *
     * @param outTradeNo       商户系统内部的订单号,32个字符
     * @param appid
     * @param mch_id
     * @param body             商品简单描述，
     * @param total_fee        订单总金额，单位为分，
     * @param spbill_create_ip 必须传正确的用户端IP,
     * @return
     */
    public static Map<String, String> h5Unifiedorder(String outTradeNo, String appid, String mch_id, String body, int total_fee, String spbill_create_ip) {
        ...
        // params.put("scene_info", "{\"h5_info\": {\"type\":\"Wap\",\"wap_url\": \"https://test.saiwuquan.com\",\"wap_name\": \"西柚商城\"}}");
        params.put("scene_info", "{\"h5_info\": {\"type\":\"Wap\",\"wap_url\": " + config.getSiteUrl() + ",\"wap_name\": \"西柚商城\"}}");
